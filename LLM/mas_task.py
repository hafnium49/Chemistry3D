...
class Chem_Lab_Task_SL(BaseTask):
    ...
    def __init__(self, name):
        ...
        self.Frankas = []
        self.Frankas_num = 7
        self._beaker_Kmno4_position = np.array([-2.807, -1.34, 0.1]) #np.array([-2.757, -1.34, 0.1])
        self._beaker_Fecl2_position = np.array([-2.662, -1.34, 0.1]) #np.array([-2.652, -1.34, 0.1])
        self._beaker_Fecl2_Return_position = self._beaker_Fecl2_position #np.array([-2.97, -1.13, 0.1])
        ...
        self._Bottle_Fecl2_position = np.array([-2.21, -1.34, 0.1]) #np.array([-2.16, -1.34, 0.1])
        self._pour0_offset = np.array([-0.01, 0.00, 0.155]) #np.array([0.08, 0.00, 0.125])
    ...
    def get_observations(self):
        ...
        beaker_Kmno4_pour_position = np.add(beaker_Kmno4_position, self._pour0_offset) #np.add(beaker_Fecl2_position, self._pour0_offset) #self._pour1_offset)
        beaker_Fecl2_pour_position = np.add(beaker_Fecl2_position, self._pour0_offset) #np.add(beaker_Kmno4_position, self._pour0_offset) #self._pour1_offset) # new
